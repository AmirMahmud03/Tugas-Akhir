...
if (isset($_GET['id'])) {
    if (!empty($_POST)) {
        ...
        // Update the record
        $stmt = $pdo->prepare('UPDATE kontak SET id = ?, nama = ?, email = ?, notelp = ?, jeniskelamin = ? WHERE id = ?');
        $stmt->execute([$id, $nama, $email, $notelp, $jeniskelamin, $_GET['id']]);
...
<div class="content update">
    ...
    <form action="update.php?id=<?=$contact['id']?>" method="post">
        ...
        <label for="jeniskelamin">Jenis Kelamanin</label>
        <input type="text" name="jeniskelamin" value="<?=$contact['jeniskelamin']?>" id="title">
